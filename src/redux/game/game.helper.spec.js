...
import { scoreDice, hasScoringDice } from './game.helper';

const getArrayOfBools = (bool, length) => Array.from(new Array(length)).map(() => bool);
describe('Game Helper tests', () => {
  describe('#scoreDice', () => {
    it('should score three ones as 1000', () => {
      expect(scoreDice([1, 1, 1], getArrayOfBools(true, 3))).to.equal(1000);
    });

    it('should score three sets of any number other than 1 as 100 * the number', () => {
      expect(scoreDice([3, 3, 3], getArrayOfBools(true, 3))).to.equal(300);
    });

    it('should score a single one as 100', () => {
      expect(scoreDice([1], getArrayOfBools(true, 1))).to.equal(100);
    });

    it('should score a single five as 50', () => {
      expect(scoreDice([5], getArrayOfBools(true, 1))).to.equal(50);
    });

    it('should score a straight as 500', () => {
      expect(scoreDice([1, 2, 3, 4, 5, 6], getArrayOfBools(true, 6))).to.equal(500);
    });

    it('should score 3 pairs as 1000', () => {
      expect(scoreDice([1, 1, 2, 2, 3, 3], getArrayOfBools(true, 6))).to.equal(1000);
    });

    it('should score a set of non-scoring dice as 0', () => {
      expect(scoreDice([2, 4, 3, 4, 2], getArrayOfBools(true, 5))).to.equal(0);
    });

    it('should score a straight that is less than 6 as 0', () => {
      expect(scoreDice([2, 3, 4], getArrayOfBools(true, 3))).to.equal(0);
    });
  });
